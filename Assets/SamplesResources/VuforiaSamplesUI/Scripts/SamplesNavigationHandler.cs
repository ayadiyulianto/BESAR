...
public class SamplesNavigationHandler : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyUp(KeyCode.Escape))
        {
            ...
            if (currentSceneName == "Vuforia-1-Menu1" && !SamplesMainMenu.isAboutScreenVisible)
            {
                #if UNITY_EDITOR
        ...
        if (Input.GetKeyUp(KeyCode.Return) || Input.GetKeyUp(KeyCode.JoystickButton0))
        {
            ...
            if (currentSceneName == /*"Vuforia-2-About"*/ "Vuforia-1-Menu1" && SamplesMainMenu.isAboutScreenVisible)
            {
